...
CREATE TABLE 학생2(
	학번 CHAR(4) NOT NULL,
	이름 VARCHAR (20) NOT NULL,
	주소 VARCHAR(50) DEFAULT '미정', --my sql에서 DEFAULT 가능
	학년 INT NOT NULL,
	나이 INT NULL,
	성별 CHAR(1) NOT NULL,
	휴대폰번호 CHAR(13) NULL,
...
ALTER TABLE 학생2
	ADD COLUMN 등록날짜 DATE NOT NULL DEFAULT '2025-04-08';  -- 열 추가
...
CREATE USER supermanager WITH PASSWORD 'krypto';  --ckrypto (비트코인), krypto(슈퍼맨의 개)
GRANT ALL PRIVILEGES ON DATABASE univdb25 TO supermanager; --DB에서만 권한 부여
GRANT ALL PRIVILEGES ON 학생2, 수강2, 과목2 TO supermanager; --TABLE에서도 권한 부여

ALTER DATABASE univdb25 OWNER TO supermanager; --소유자도 변경하면 모든 권한이 있음
