...
1. 'WITH GRANT OPTION을 함께 명세하면 부여받은 권한을 계정 사용자가 다른 사용자 계정에게도 부여할 수 있는 권한을 갖게 된다. (o_)
...
2. 계정에게 부여한 권한은 (REVOKE_) 명령문을 통해 다시 철회할 수 있다.
3. 뷰의 장점은 편의성, (보안성), 재사용성, 독립성이다.
4. 대부분의 DBMS는 (B-트리__) 구조의 인덱스를 지원한다.
...
CREATE VIEW 장학생 AS
SELECT DISTINCT s.학번, s.이름, s.학년, s.휴대폰번호
FROM 학생 s
...
<단점>
1. 저장 공간 추가 필요
2. 삽입/삭제/수정 시 인덱스도 함께 갱신되어 성능 저하 가능
