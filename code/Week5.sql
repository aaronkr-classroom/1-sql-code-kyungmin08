...
WHERE (학년 >= 1 AND 학년 <= 3) OR NOT (소속학과 = '컴퓨터'); -- BETWEEN 1 AND 3도 괜찮음

SELECT 이름, 학년, 소속학과 FROM 학생
WHERE 소속학과 = '컴퓨터' OR 소속학과 = '정보통신' --WHERE 소속학과 = '컴퓨터' OR '정보통신' (X)
ORDER BY 학년 ASC; 
...
--JOIN
SELECT * FROM 학생 CROSS JOIN 수강; --모든 행 결합

SELECT * FROM 학생 JOIN 수강 ON 학생.학번 = 수강.학번; --동등 조인

SELECT S1.이름, S2.이름 -- S1, S2은 별명
