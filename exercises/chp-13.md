...
2. db.회원.updateMany({}, {$set: {탈퇴날짜: null}});__
3. db.회원.drop()
...
NoSQL 데이터베이스는 크게 네 가지 유형으로 구분된다. 문서 지향 데이터베이스는 JSON이나 BSON 형식의 문서를 저장하며, 유연한 스키마와 계층적 구조를 지원하여 다양한 데이터를 손쉽게 처리할 수 있다. 키-값 저장소는 단순한 키-값 쌍으로 데이터를 저장하여 빠른 읽기·쓰기 성능을 제공하며, 캐시나 세션 관리에 적합하다. 열 지향 데이터베이스는 같은 열의 데이터를 함께 저장하여 대용량 분석 작업에 유리하며, 주로 데이터 웨어하우스에 사용된다. 그래프 데이터베이스는 노드와 엣지를 기반으로 복잡한 관계를 표현하고 탐색하는 데 적합하여 소셜 네트워크나 추천 시스템에 효과적이다.

**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**
MongoDB는 문서 기반의 NoSQL 데이터베이스로서, 고정된 스키마 없이 JSON 형태의 문서를 저장하며 유연하고 확장성이 뛰어나다. 반면, 관계형 데이터베이스는 테이블 구조와 고정된 스키마를 바탕으로 SQL을 통해 데이터를 처리하고, 복잡한 관계와 트랜잭션을 정교하게 관리하는 데 적합하다. MongoDB는 빠른 개발과 비정형 데이터 처리에 유리하며, 관계형 데이터베이스는 데이터 무결성과 복잡한 질의에 더 적합하다.
